...
- [ ] 1.1 Create `backend/app/analytics/` package with `__init__.py`.
- [ ] 1.2 Create `backend/app/analytics/schemas.py` defining Pydantic DTOs:
...
- [ ] 2.1 Create `backend/app/analytics/service.py` with `AnalyticsService` class.
- [ ] 2.2 Implement `get_order_metrics(session, business_id: int) -> OrderMetricsDTO`:
...
- [ ] 2.3 Implement `get_product_metrics(session, business_id: int) -> ProductMetricsDTO`:
...
- [ ] 3.1 Implement `get_inquiry_metrics(session, business_id: int) -> InquiryMetricsDTO`:
  - Group and count by `status`.
  - Fetch 5 most recent inquiries (order by `created_at DESC, id DESC`).
  - Enforce `business_id` filter.
- [ ] 3.2 Implement `get_customer_metrics(session, business_id: int) -> CustomerMetricsDTO`:
...
- [ ] 3.3 Implement `get_feedback_metrics(session, business_id: int) -> FeedbackMetricsDTO`:
  - Group and count by `sentiment`.
  - Enforce `business_id` filter.
- [ ] 3.4 Implement `get_business_analytics_report(session, business_id: int) -> BusinessAnalyticsReportDTO`:
...
- [ ] 4.3 Write `tests/test_analytics_null_semantics.py`:
...
- [ ] 4.4 Write `tests/test_analytics_status_filtering.py`:
  - Verify that only `confirmed` orders contribute to revenue metrics and product quantities.
- [ ] 4.5 Write `tests/test_analytics_empty_data.py`:
  - Verify that querying an empty database returns 0 for counts, empty lists for groupings/recent, and `Decimal("0")` for revenue.

## 5. Verification

- [ ] 5.1 Run `openspec validate business-analytics-foundation` and ensure no errors.
- [ ] 5.2 Verify `AnalyticsService` performs SELECT/read operations only and does not modify `Order`, `OrderItem`, `Inquiry`, `Customer`, `Feedback`, raw WhatsApp records, relevance assessments, or extraction data.
- [ ] 5.3 Verify every analytics query is scoped to the requested `business_id`, including `OrderItem` aggregation through the parent `Order`.
- [ ] 5.4 Verify monetary and quantity DTO fields use `Decimal` and analytics never converts database Numeric money/quantity values to `float`.
- [ ] 5.5 Verify NULL monetary values are never converted to zero before aggregation or presented as known revenue.
- [ ] 5.6 Verify analytics contains no Gemini/LLM, LangChain, RAG, embedding, vector-database, raw-message interpretation, extraction, or identity-resolution logic.
- [ ] 5.7 Run the COMPLETE existing backend regression suite (`pytest`) and verify ingestion, business relevance detection, and AI extraction remain passing alongside new analytics tests.
