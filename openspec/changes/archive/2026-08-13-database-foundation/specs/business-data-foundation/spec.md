...
This capability establishes the following persistent data entities for the MVP:
...
The capability SHALL provide the relationships and database constraints necessary for these entities to form a coherent business-data model.

The capability SHALL NOT implement the workflows that populate these entities from WhatsApp exports or AI processing. Those workflows SHALL be implemented by subsequent OpenSpec changes.

---

## Acceptance Criteria

The `business-data-foundation` capability SHALL be considered complete when:

- A new SQLite database can be initialized successfully.
- All required MVP entities can be created through SQLAlchemy models.
- Required relationships between entities are defined.
- Foreign-key and integrity constraints are enforced where applicable.
- Required uniqueness constraints and indexes are implemented.
- Business ownership can be determined for business-owned records.
- Raw messages can be persisted and retrieved.
- Media metadata can be persisted and retrieved.
- AI-derived records can be persisted separately from raw messages.
- Derived records can reference one or more source messages.
- Extraction confidence, status, and model metadata can be stored.
- Human-review state can be represented.
- Invalid foreign-key relationships cannot be persisted.
- Related database operations maintain transaction integrity.
- Database sessions can be created and managed reliably.
- Basic persistence tests pass.
- The resulting schema is ready to support the subsequent `whatsapp-export-ingestion` capability.